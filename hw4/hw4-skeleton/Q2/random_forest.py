...
class RandomForest(object):
    ...
    def _bootstrapping(self, XX, n):
        ...
        sample_indexes = np.random.randint(0,n,size = n)
        ...
        for record in X:
            ...
            if len(counts) == 0:
                ...
                for tree in self.decision_trees:
                    cur_votes.append(tree.classify(record))
                    cur_counts = np.bincount(cur_votes)
                    y = np.append(y, np.argmax(cur_counts))
...
def main():
    ...
    numerical_cols=set([i for i in range(0,43)]) # indices of numeric attributes (columns)
